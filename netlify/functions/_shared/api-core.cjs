...
const dataDir = isNetlifyFunction ? path.join("/tmp", "viralscope") : path.join(root, "data");
...
async function loadDbFromBlob() {
  ...
  try {
    ...
    const db = await store.get(blobDbKey, { type: "json" });
    if (db) {
      fs.writeFileSync(dbPath, JSON.stringify(db, null, 2));
      return;
    }
...
function sendJson(res, status, body, extraHeaders = {}) {
  res.writeHead(status, {
    ...
    "Access-Control-Allow-Origin": process.env.CORS_ORIGIN || appUrl,
...
async function handleApi(req, res, url) {
  ...
  if (url.pathname === "/api/billing/confirm" && req.method === "POST") {
    ...
    try {
      ...
      if (!updatedUser) {
        sendJson(res, 404, { error: "Checkout completed, but the matching CommandCue account was not found." });
        return;
      }
